...
\`\`\``],answerHtml:["<p><strong>是一条从上下文收集 → 语言模型推断 → 候选生成与重排 → 约束与安全 → 呈现与学习的工程流水线</strong></p>",`<div class="language-swift"><button title="Copy Code" class="copy"></button><span class="lang">swift</span><pre><!--::markdown-it-async::lt43xp7zmkqq4bcgtelpej::--><code>键入 → 触发条件命中
   → 上下文构建（prefix+suffix+符号/RAG+工程规则）
     → 模型解码（FIM/自回归，KV cache，限长/停用符）
       → 生成多候选（不同温度/窗口）
         → 重排（可编性/风格/相似度/历史采纳）
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::tk6749rb58cftl7lt630ew::--><code>
function bubbleSort(arr) {
  for (let i = 0; i &lt; arr.length - 1; i++) {
    for (let j = 0; j &lt; arr.length - 1 - i; j++) {
      if (arr[j] &gt; arr[j + 1]) {
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::v3bvutvnb1zyj0wvk5ydf::--><code>function selectionSort(arr) {
  for (let i = 0; i &lt; arr.length; i++) {
    let min = i;
    for (let j = i + 1; j &lt; arr.length; j++) {
      if (arr[j] &lt; arr[min]) min = j;
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::sh8zf1f3lfcveanas0gyf::--><code>function insertionSort(arr) {
  for (let i = 1; i &lt; arr.length; i++) {
    let key = arr[i];
    let j = i - 1;
    while (j &gt;= 0 &amp;&amp; arr[j] &gt; key) {
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::9re9m10btvx98308mc3yj::--><code>function quickSort(arr) {
  if (arr.length &lt;= 1) return arr;
  const pivot = arr[arr.length &gt;&gt; 1];
  const left = arr.filter(x =&gt; x &lt; pivot);
  const mid = arr.filter(x =&gt; x === pivot);
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::pbeynxnmbq7750ikgrq357::--><code>
function mergeSort(arr) {
  if (arr.length &lt;= 1) return arr;
  const mid = Math.floor(arr.length / 2);
  const left = mergeSort(arr.slice(0, mid));
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::tggvsatm2mc5fefdke76xf::--><code>function countingSort(arr, maxValue) {
  const count = new Array(maxValue + 1).fill(0);
  arr.forEach(v =&gt; count[v]++);
  const res = [];
  count.forEach((v, i) =&gt; { while (v--) res.push(i); });
...
\`\`\``],answerHtml:[`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::iqb8r1hqrggx8h3wpxtdc::--><code>function shuffle(array) {
  let arr = array.slice(); // 拷贝一份，不修改原数组
  for (let i = arr.length - 1; i &gt; 0; i--) {
    const j = Math.floor(Math.random() * (i + 1)); // 随机索引 [0, i]
    [arr[i], arr[j]] = [arr[j], arr[i]]; // 交换
...
<li>过程中记录窗口最大长度。</li>
</ul>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::ada7s3wjzuf45t3ew2rixv::--><code>function lengthOfLongestSubstring(s) {
  let set = new Set();
  let left = 0, maxLen = 0;
...
基于 ES6 Proxy 拦截子应用对全局对象的读写。每个子应用有一个独立的代理对象作为 window，实际访问 window.xxx 时，会优先读写子应用自己的代理上下文。多个子应用可并行运行，互不影响。</p>`,"<p>qiankun 默认在支持 Proxy 的浏览器下使用这个方案</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::1a9daj83rheevigo7e3nj::--><code>const fakeWindow = {};
const proxy = new Proxy(window, {
        get(target, key) {
        return key in fakeWindow ? fakeWindow[key] : target[key];
},
...
\`\`\``],answerHtml:[`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::oa0hj2u0xspqbdnb56v6mi::--><code>import type { Plugin } from &#039;vite&#039;;
...
</div>`,"<p>// 替换模板的一个字</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::mkxb0oenfhkogqbqd3d7bf::--><code>export default function replaceTextPlugin(options = { search: &#039;FOO&#039;, replace: &#039;BAR&#039; }) {
  return {
    name: &#039;vite-plugin-replace-text&#039;,
    enforce: &#039;pre&#039;,
    transform(code, id) {
...
</div>`,"<p>// 统计TODO</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::1rnwnsugvrrgo8qrdz21l7::--><code>import fs from &#039;fs&#039;;
import path from &#039;path&#039;;
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::48jr29k4rvpsewl4x6h66::--><code>function isElementRendered(sampleCount = 10) {
    let visibleCount = 0;
    for (let i = 0; i &lt; sampleCount; i++) {
        const x = Math.random() * window.innerWidth;
        const y = Math.random() * window.innerHeight;
...
<li>重复上述步骤</li>
</ul>`,"<p>宏任务 → 微任务 → ✅ 渲染 → 下一个宏任务</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::imvv44u7cyto2glpkmu0oq::--><code>┌────────────────────────────┐
│       1. 取一个宏任务       │ (如 script, I/O, setTimeout 回调)
├────────────────────────────┤
│       2. 执行所有微任务     │ (Promise.then, MutationObserver)
├────────────────────────────┤
...
</ul>`,"<p>典型用法:</p>",`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::ixiqbvfwo8f488ysc52bek::--><code>&lt;Form form={form} initialValues={{ username: &#039;Tom&#039; }}&gt;
  &lt;Form.Item name=&quot;username&quot;&gt;
    &lt;Input /&gt;
  &lt;/Form.Item&gt;
&lt;/Form&gt;</code></pre>
</div>`,"<p>对应关系：</p>",`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::x2j6q55j9qzx8cd0o9ip::--><code>&lt;Form&gt;           —— 维护 FormStore（字段状态）
   │
   └─&lt;FormContext.Provider&gt;  —— 提供 Store 和订阅方法
        │
        └─&lt;Form.Item name=&quot;username&quot;&gt; —— 注册字段
...
</ul>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::ll99um0ca0mtwr59emd8r9::--><code>    {
    value: store[name],
    onChange: (e) =&gt; {
      const newValue = e &amp;&amp; e.target ? e.target.value : e;
      store.setFieldValue(name, newValue);
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::ifxn1winzidzbygshhpi9::--><code>const arr = [
  { id: 1, name: &quot;i1&quot; },
  { id: 2, name: &quot;i2&quot;, parentId: 1 },
  { id: 4, name: &quot;i4&quot;, parentId: 3 },
  { id: 3, name: &quot;i3&quot;, parentId: 2 },
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::v4cmkshmo4gjpn015tba4r::--><code>function arrange(name) {
  const queue = []
...
\`\`\``],answerHtml:[`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::ewykqius4x39ecsk9wq8n::--><code>// 代码由github copilot 生成
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::zysf4s2a7xxppgikor3fl::--><code>
function debounce(fn, wait = 300) {
  let timer = null;
  return function(...args) {
    clearTimeout(timer);
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::2jaugjmqtqxci06pq0m2nt::--><code>
function throttle_timestamp(fn, wait) {
  let last = 0; // 上次执行时间（ms）
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::qiftq28ycpb3dm2k90pw29::--><code>class MyPromise {
        static PENDING = &quot;pending&quot;;
        static RESOLVED = &quot;resolved&quot;;
        static REJECTED = &quot;rejected&quot;;
        constructor(executor) {
...
</ul>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::6lf9yhn2vcqvx0uxcqhclj::--><code>// 简单版
Function.prototype.myBind = function (context) {
  const fn = this; // 保存原函数
  return function () {
    return fn.apply(context);
...
</table>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::88l34c0yaueduud4b3k3b8::--><code>function greet(g1, g2) {
  console.log(this.name, g1, g2);
}
...
</ul>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::mab441qpnmqa6zycjodhdh::--><code>function myInstanceof(left, right) {
  let proto = Object.getPrototypeOf(left), // 获取对象的原型
      prototype = right.prototype; // 获取构造函数的 prototype 对象
...
\`\`\``],answerHtml:["<p>在调用 new 的过程中会发生以上四件事情：</p>","<p>（1）首先创建了一个新的空对象</p>","<p>（2）设置原型，将对象的原型设置为函数的 prototype 对象。</p>","<p>（3）让函数的 this 指向这个对象，执行构造函数的代码（为这个新对象添加属性）</p>","<p>（4）判断函数的返回值类型，如果是值类型，返回创建的对象。如果是引用类型，就返回这个引用类型的对象。</p>",`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::26burw773jgbi0rsyxoijd::--><code>
function myNew(constructor, ...args) {
  if (typeof constructor !== &quot;function&quot;) return
  let obj = {}
  obj.prototype = Object.create(constructor.prototype)
...
\`\`\``],answerHtml:["<p>柯里化（Currying） 是把一个接受多个参数的函数，转换成一系列每次只接收一个参数的函数。</p>","<p>类比: “一次性买 3 个苹果”，变成 “一次买 1 个，买 3 次，最后结算”。</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::2gbwz7t0di6fdgmnacgu8p::--><code>function add(a, b, c) {
  return a + b + c;
}
...
<li>✅ 提升代码可读性和函数式风格</li>
</ul>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::n19cizfkpfjn9zj6y9tcgo::--><code>
function curry(fn) {
  const curried = (...args) =&gt; {
    // 如果参数足够就执行，否则返回新函数继续收集
    return args.length &gt;= fn.length
...
\`\`\``],answerHtml:["<p>一、页面可见性 Page Visibility API</p>","<p>判断用户标签页是否可见（切换到后台/最小化浏览器）</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::yrmg4eik2kvzn5xfr3ktr::--><code>document.addEventListener(&#039;visibilitychange&#039;, () =&gt; {
  if (document.hidden) {
    console.log(&#039;用户离开页面（切换标签页或最小化）&#039;);
  } else {
    console.log(&#039;用户回到页面&#039;);
...
</div>`,"<p>二、窗口焦点（Focus / Blur）</p>","<p>检测窗口或 iframe 是否获得焦点</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::d34vbujiwzfqmotmwc0g4::--><code>window.addEventListener(&#039;blur&#039;, () =&gt; {
  console.log(&#039;用户离开了当前窗口/标签页&#039;);
});
...
</div>`,"<p>三、用户离开页面 / 卸载（beforeunload / unload）</p>","<p>检测用户关闭页面、刷新或跳转。</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::wbhd29y6jllml2p20grub::--><code>window.addEventListener(&#039;beforeunload&#039;, (e) =&gt; {
  console.log(&#039;用户可能离开页面&#039;);
  // 阻止默认提示（可选）
  e.preventDefault();
  e.returnValue = &#039;&#039;;
...
</div>`,"<p>四、鼠标 / 用户交互检测</p>","<p>当用户长时间没有交互（鼠标、键盘、触屏），可推测用户离开</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::bw2xkjet0ak1arspzb8gh6::--><code>let idleTimer;
function resetIdle() {
  clearTimeout(idleTimer);
  idleTimer = setTimeout(() =&gt; {
    console.log(&#039;用户可能离开或无操作&#039;);
...
</div>`,"<p>可以检测“用户不活跃”，用于自动登出、心跳等.</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::etc2ms3hcjou87q7t4edb::--><code>let idleTimer;
function onUserIdle() { console.log(&#039;用户离开页面或长时间不操作&#039;); }
function resetIdle() {
  clearTimeout(idleTimer);
  idleTimer = setTimeout(onUserIdle, 30000);
...
<li>构建观察者 / 订阅机制</li>
<li>动态执行规则</li>
</ol>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::dji7ry1nan65yibocqem8o::--><code>formFields = [
  { field: &#039;province&#039;, type: &#039;select&#039; },
  { field: &#039;city&#039;, type: &#039;select&#039;, dependsOn: &#039;province&#039;, compute: (state) =&gt; cities[state.province] }
]
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::j2c5sstk56kvl3hl522wp::--><code>let lastRequestId = 0;
...
</ol>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::6yuy5ml739xpfmqhfgykxp::--><code>useEffect(() =&gt; {
  let canceled = false;
...
</table>`,"<p>Vue 3 自带了<code> &lt;KeepAlive&gt;</code> 组件</p>","<p>react 实现 keep-alive</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::p4afgwo9h7dvtxi7u5zhfr::--><code>// KeepAlive.jsx 
import { useRef } from &#039;react&#039;;
import { useLocation } from &#039;react-router-dom&#039;;
...
</ul>`,"<p>js sdk</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::gjlyul1g72ma14z1rdwjh5::--><code>// perf-sdk.js (浏览器端核心逻辑, 精简示例)
(function (window) {
  const APP = {};
  const SEND_URL = &#039;/beacon/perf&#039;;
  const BATCH_MAX = 20;
...
\`\`\``],answerHtml:["<p><strong>当主线程（Main Thread）执行某个任务超过 50ms</strong>，它就被标记为一个 “Long Task”。</p>","<p>浏览器提供了 PerformanceObserver 接口，监听类型为 &quot;longtask&quot; 的性能条目。</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::s8agqiiie4qzpn7tefq9::--><code>
let longTaskCount = 0;
let totalLongTaskTime = 0;
...
\`\`\``],answerHtml:[`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::z4wfgav5d1skmvan0q39p::--><code>
组件库发布
   │
   ├─ 生成 CHANGELOG
   ├─ 更新远程版本记录
...
</table>`,`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::l3drqmut5vnx3e2uct01i::--><code>// 模块表（缓存）
const modules = {};      // 模块名 -&gt; 导出对象
const factories = {};    // 模块名 -&gt; 模块定义函数
const loading = {};      // 模块名 -&gt; 是否正在加载中
...
</div>`,`<div class="language-scss"><button title="Copy Code" class="copy"></button><span class="lang">scss</span><pre><!--::markdown-it-async::q8vs9mlckmnfn7krduo0t::--><code>require(&#039;main&#039;)
   │
   ├─▶ 检查缓存
   │
   ├─▶ 找到定义 (factory)
...
</table>`,"<p>实践中采用 OpenSeadragon 库</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::57j1aygzd93n0ahbhwcyqb::--><code> ┌─────────────────────────────┐
 │         大图瓦片浏览器         │
 │                             │
 │  ┌──────────────┐            │
 │  │视窗视口(Viewport)│  ← 仅渲染屏幕内的瓦片 │
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::je66qzov12sppdrxn5d1::--><code>[宏任务/事件] → [微任务] → [rAF 回调] → [样式计算/布局/合成/绘制] → [空闲回调/下一轮]</code></pre>
...
\`\`\``],answerHtml:[`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::x7qsl3lt0v2807s8l58fq::--><code>async function batchRequestAllSettled(tasks, batchSize = 5) {
  const results = []
...
</ul>`,"<p>import 导入的变量是 活引用（live binding），不是拷贝。</p>",`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::0irutfcc09pdu5ce1r8hh18::--><code>// counter.js
export let count = 0;
export function inc() { count++; }
...
</ol>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::e9rfbgh6kghap2ph33o96i::--><code>每个阶段的宏任务执行完 →
    执行所有 nextTick 回调 →
    执行所有微任务 (Promise)
→ 进入下一个阶段</code></pre>
...
</table>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::mwqh8xue3um21i8mealjtt::--><code>客户端 (Client)                             服务器 (Server)
------------------                          ------------------
...
假设浏览器要加载三个资源：</p>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::0rjdzhw5akfejpazv42mq::--><code>index.html
style.css
main.js
</code></pre>
</div>`,"<p>HTTP/1.1：</p>",`<ul>
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::q59917w05r9qcqn6fdoh9::--><code>// HTTP/2
HTTP/2
  ↓
  TLS
  ↓
...
</ol>`,`<div class="language-nginx"><button title="Copy Code" class="copy"></button><span class="lang">nginx</span><pre><!--::markdown-it-async::qk5wwhmqg8eu8a105seq8::--><code>http {
    # 开发环境（域名区分）
    server {
        listen 80;
        server_name dev.example.com;  # 开发环境域名
...
</div>`,"<p>同域名不同路径映射的重写问题及解决方案</p>","<p>当多个前端应用部署在同一域名的不同路径下（如 example.com/app1、example.com/app2），需要解决路径映射和资源引用的问题。</p>",`<div class="language-nginx"><button title="Copy Code" class="copy"></button><span class="lang">nginx</span><pre><!--::markdown-it-async::nvllus5le7gzdy0gqijs6::--><code>server {
    listen 80;
    server_name example.com;
    root /var/www;  # 父目录
...
\`\`\``,"🔴 每次点击按钮，Child 都会重新挂载，副作用也会重新执行。","当父组件内部定义子组件时，每次父组件 render 都会创建一个新的函数对象。React 看到“类型变了”，认为它是一个全新的组件。结果：子组件每次都会被卸载并重新挂载。**可以使用 useCallback 优化**",`- 什么时候可以在内部定义组件?
  - 组件依赖父组件的闭包变量
  - 性能无关的小组件
  - 需要动态生成组件类型`],answerHtml:[`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::wkjpad0rqi2xibtz5170n::--><code>function App() {
  const [count, setCount] = useState(0);
...
- 支持自定义比较，通过第二个参数实现更精细的控制`],answerHtml:["<p><strong>React.memo 是一个高阶组件（HOC），用于优化函数组件的渲染性能。它的核心原理是通过记忆（Memoization）组件的渲染结果，在 props 未变化时跳过重新渲染，直接返回上一次的渲染结果。</strong></p>",`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::iwnu8gebq5w8my138bfbe::--><code>// React.memo 的简化实现逻辑
function memo(Component, arePropsEqual) {
  let lastProps = null;
  let lastResult = null;
...
- 启用 ESLint 规则自动检测
- 使用函数式更新避免依赖状态值`],answerHtml:["<p>表现:缺少依赖导致过时闭包</p>",`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::hy18y6qpyvd3xnhk3jb2xg::--><code>function Timer() {
  const [count, setCount] = useState(0);
...
比如：读取 DOM 布局信息（测量元素大小）;需要在绘制前同步修改 DOM 样式（避免闪烁)</p>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::nwgcb6hhm3dah0bvnbpjk4::--><code>Render 阶段（计算虚拟 DOM）
        ↓
DOM 更新（Commit 阶段）
        ↓
useLayoutEffect（同步执行） 👈 页面还没绘制
...
把原本递归的更新过程变成可中断的循环（迭代），并用一个 Fiber 数据结构 表示组件树中的每个节点。
每个 Fiber 节点本质上是一个 JS 对象.对应一个 React element.</p>`,`<div class="language-jsx"><button title="Copy Code" class="copy"></button><span class="lang">jsx</span><pre><!--::markdown-it-async::hb14mdwm82jeaim5iebbft::--><code>type Fiber = {
  type: any,             // 组件类型（函数/类/DOM标签）
  stateNode: any,        // 对应的DOM实例或类组件实例
  child: Fiber | null,   // 第一个子Fiber
  sibling: Fiber | null, // 下一个兄弟Fiber
...
React 通过 Lane 模型 管理任务优先级</p>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::2vv9r42t34m1devpktkmc::--><code>  // 优先级从高到低
ImmediatePriority（用户输入）
UserBlockingPriority（悬停、点击）
NormalPriority（数据请求）
LowPriority（分析日志）
...
</ul>`,"<p>👉 Concurrent Rendering（并发渲染） 就是 React Fiber 在 16 以后引入的核心能力，真正实现在 渲染过程中可以打断、暂停、恢复、丢弃。</p>","<p>旧:</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::a0d2wy1zxeht06cioy8x::--><code>update → render → commit
         (整棵树一次性调和)</code></pre>
</div>`,"<p>新:</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::i6t65syvpu0q06o4wnfa4f::--><code>update → render(可被打断、分段) → commit(同步)</code></pre>
</div>`,`<ul>
...
</ul>`,"<p>在 Fiber 节点上，每个函数组件会维护一个 <strong>Hook 链表</strong>：</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::pwi7imfie2zbrpmxaybz::--><code>type Hook = {
  memoizedState: any,        // 保存副作用依赖数组
  next: Hook | null          // 指向下一个 Hook
}
...
\`\`\``],answerHtml:[`<div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre><!--::markdown-it-async::ai6azw1p408v9kvs2iri0s::--><code>
function Demo() {
  const [count, setCount] = useState(0)
...
</div>`,`<div class="language-txt"><button title="Copy Code" class="copy"></button><span class="lang">txt</span><pre><!--::markdown-it-async::jthw5odc5loqkuemtwomb::--><code>┌─────────────── 一次“点击触发更新”的完整时间线 ───────────────┐
...
</ul>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::6vn5l5zzgi8zdf6aedw62t::--><code>function useState(initialState) {
    return useReducer((state, action) =&gt; {
        return typeof action === &#039;function&#039; ? action(state) : action;
    }, initialState);
}</code></pre>
...
</ol>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::03jjxs53pmnrc3ub7uese3a::--><code># CODEOWNERS
/apps/web/*          @frontend-team
/apps/admin/*        @admin-team
/packages/utils/*    @core-lib-team
...
<li>保证更新最少、性能最好。</li>
</ul>`,"<p>Vue 3 的 nextTick 实际上是封装了一个<strong>微任务调度器（microtask scheduler）</strong>。</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::n7d1nvm4hor34nvnudzoa4::--><code>// Vue3 内部核心实现伪代码
let pending = false
const queue = []
...
3. 代码生成: 生成渲染函数是编译的最终阶段，这个阶段会将优化后的 AST 转换成 JavaScript 渲染函数。`],answerHtml:["<p><strong>模板解析、AST优化 和 代码生成</strong></p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::9xw54u1ards0juhobulievs::--><code>模板字符串 (template)
       ↓
[1] 解析（Parse） → 生成 AST
       ↓
[2] 转换（Transform） → 优化、静态标记、指令处理
...
✅ 避免频繁的 DOM 重绘（性能优化）
✅ 保证多次修改只渲染一次（批量更新）
✅ 数据一致性：确保在同一事件循环中的所有数据变更后，视图一次性更新到最终状态。</p>`,`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::3lt6f4s9tq6hfh02j8uvke::--><code>数据变化（Proxy.set）
       ↓
触发依赖（trigger）
       ↓
派发更新任务（scheduler → queueJob）
...
\`\`\``],answerHtml:[`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::1lvjojel8el25xlm844dkg::--><code>
&lt;template&gt;
  &lt;div&gt;
    &lt;h2 v-once-directive&gt;{{ message }}&lt;/h2&gt;
    &lt;button @click=&quot;message = &#039;更新后的消息&#039;&quot;&gt;修改 message&lt;/button&gt;
...
</div>`,"<p>事件</p>",`<div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre><!--::markdown-it-async::6li4ed1iojekgshcyhsmf::--><code>
&lt;template&gt;
  &lt;button v-once-click=&quot;handleClick&quot;&gt;点击我一次&lt;/button&gt;
&lt;/template&gt;
...
</div>`,"<p>自定义指令有很多钩子可以选择</p>",`<div class="language-vue"><button title="Copy Code" class="copy"></button><span class="lang">vue</span><pre><!--::markdown-it-async::ndyep42g08e9038fku4jcl::--><code>const myDirective = {
  // 在绑定元素的 attribute 前
  // 或事件监听器应用前调用
  created(el, binding, vnode) {
    // 下面会介绍各个参数的细节
