...
在 Chrome DevTools 的 Memory（内存）→ Heap snapshot 里：

- 浅层大小（Shallow Size）

- 保留大小（Retained Size）

都是针对“某个对象”算出来的两个不同概念。

可以直接把它们想成：

 - 浅层大小 = 这个对象自己有多大(对象本身占用的内存)
 - 保留大小 = 因为它还活着，连带导致不能被回收的那一整团对象有多大

**小技巧**: 在右上角 Filter 里输入 detached，可以快速找到“脱离 DOM 但还在内存里的节点”，这是内存泄漏常见来源（事件监听没解绑、闭包引用 DOM 等）
