...
### 9. 有Agent 应该够了吧, 为什么要向量知识库呢

<Collapse>

就算有 Agent，也离不开“知识来源”——把静态资料做成向量库，其实就是把药品说明、库存策略等内容塞进一个“可检索的数据库”，Agent 在需要时可以即时查到准确原文，而不是凭记忆或胡编。这样有几个好处：

- 解决“模型记不住细节”的问题
  - 纯 LLM 只靠参数记忆，很难覆盖你医院或药房的私有数据；有了向量库，Agent 能随时检索最新的药品说明、库存规则并引用，避免幻觉。

- 支持实时更新
  - 药品库存、使用指引会频繁变化，维护一份向量知识库（或接入真实 DB）就能随时更新，而不用每次都重新训练模型。

- 提高可追溯性
  - Agent 调用 drug_rag_search 时，我们能记录它引用了哪些文档、哪些药品，方便追踪和解释“回答依据”。

- 便于扩展场景
  - 未来要加入更多药品、政策、指南，只需往知识库里追加文档，Agent 立即可以使用，不用改 prompt 或模型。

因此，Agent 和向量库是互补关系：Agent 负责“读问题、决定流程”，向量库负责“提供准确知识”；两者配合才能在真实医院环境里给出可信、可审计的答案。
...
| 参数                                    | 含义                     | 常见取值/默认             | 主要作用                 | 何时怎么调                                              |
| ------------------------------------- | ---------------------- | ------------------- | -------------------- | -------------------------------------------------- |
| **Temperature**                       | 采样温度，控制分布“平坦/尖锐”       | 0.0–1.5（常用 0.2–1.0） | 越高越发散、创意多；越低越保守、确定性强 | **事实性/确定答案**降到 0.2–0.7；**创意写作/脑暴**升到 0.9–1.2       |
| **Top-K**                             | 只在概率最高的前 K 个 token 中采样 | 0（关闭）或 20–100       | 数量截断：限制搜索空间，减少长尾噪声   | 分布很“平”的模型可设 40–100；若已用 Top-P，可把 Top-K 设较大或 0       |
| **Top-P**（Nucleus）                    | 在累计概率 ≥ P 的最小集合内采样     | 0.7–0.97（常用 0.9）    | 质量截断：自适应裁掉长尾         | 常与温度联用：`temp≈0.7~1.0 + top_p≈0.9` 是稳妥起点            |
| **Max Length** / **Max Tokens**       | 本次生成的最大 token 数        | 64–8192+（依模型上下文）    | 限制回复长度，防失控输出         | 问答短回复设 64–256；长文案/代码适度放大，注意上下文窗口上限                 |
| **Stop Sequences**                    | 遇到任意 stop 序列即停止生成      | 字符串或数组              | 让模型在自定义分隔符处停         | 对**流式多轮**、**结构化输出**很有用，如 `["\n\nHuman:", "<END>"]` |
| **Frequency Penalty**                 | 按出现频次惩罚重复 token        | 0.0–2.0（常用 0–0.8）   | 抑制复读机，鼓励多样措辞         | 文案/摘要重复较多时升到 0.2–0.8；太高会跑题                         |
| **Presence Penalty**                  | 按是否已出现过惩罚              | 0.0–2.0（常用 0–0.8）   | 鼓励引入**新主题/新词**       | 需要更“发散/覆盖面”的场景提高到 0.2–0.8                          |
| **Repetition Penalty**（部分实现）          | 对已生成过的 token 统一降权      | 1.0–1.3（>1 惩罚）      | 降低整体重复率              | 和 Frequency/Presence 二选一或小幅搭配，避免过度叠加               |
| **Min Length** / **Min Tokens**（部分实现） | 最少生成 token 数           | 0–数百                | 防止提早截止               | 模板化输出、分段回答时用，注意与 Stop 冲突                           |
| **Seed**                              | 采样随机种子                 | 任意整数                | 复现实验结果               | 调参/AB 测试时固定，线上可不设提升多样性                             |
| **Penalty Bias / Logit Bias**（部分实现）   | 手动调某些 token/logit      | -5~+5（实现相关）         | 强制鼓励/抑制特定词           | 结构化格式、强制语言/风格时使用                                   |
| **Tools / Functions**（工具调用）           | 提供可调用函数的签名             | JSON schema         | 让模型会“用工具”            | 设计清晰的 name/参数+示例，配合系统提示约束调用策略                      |
| **Stop on Tool Call**（部分实现）           | 工具触发即停止文本生成            | 布尔                  | 明确“先工具再续写”的回合节奏      | Agent 场景更稳，避免边说边误触工具                               |


快速使用套路

- 默认保守稳妥：temperature=0.7、top_p=0.9、top_k=0 或 50、max_tokens 按场景设定。

- 更确定（考试题、算式、代码修复）：温度降到 0.2–0.5，可关 top_p/top_k 或设较严。

- 更有创意（营销文案/故事）：温度 0.9–1.2，top_p≈0.92–0.97，适度提高 presence/frequency penalty。

- 避免啰嗦/兜圈：降低 max_tokens，设置合适 stop，小幅 frequency/presence penalty。

- 结构化输出：结合 stop、少量 logit_bias 或模板提示；需要多段时配 Min/Max Length。
